...
#
if [ "$#" != "3" ]; then
    _error "Exactly three arguments required: backup storage location root, hostname and backup date to remove"
fi
...
if [ ! -e $HOST_BACKUP_INSTANCE_LOCATION ]; then
    _error "Host backup instance location not found: $HOST_BACKUP_INSTANCE_LOCATION"
fi
...
### Delete subvolume
#
echo "      POSIX: Removing $HOST_BACKUP_INSTANCE_LOCATION" &&
rm -rf $HOST_BACKUP_INSTANCE_LOCATION &&
